...
test("acknowledges an automatic release once and keeps the release center available", async ({ page }) => {
  await page.addInitScript(() => {
    if (!sessionStorage.getItem("release-test-seeded")) {
      localStorage.setItem("builder:last-seen-release", "6.2");
  ...
  await expect(announcement).toContainText("v6.2.0 → v6.3.0");
  ...
  await expect(page.getByTestId("release-center")).toContainText("v6.3.0");
